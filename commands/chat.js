...
export async function execute(remainder, registry) {
 ...
 if (response.usage) {
  ...
  chatService.systemLine(`[Chat Complete] Token usage - promptTokens: ${promptTokens}, completionTokens: ${completionTokens}, totalTokens: ${totalTokens}, cost: ${cost}`);
  if (response.timing) {
    const { elapsedMs, tokensPerSec } = response.timing;
    const seconds = (elapsedMs / 1000).toFixed(2);
    const tps = tokensPerSec !== undefined ? tokensPerSec.toFixed(2) : 'N/A';
    chatService.systemLine(`[Chat Complete] Time: ${seconds}s, Throughput: ${tps} tokens/sec`);
  }
 } else {
