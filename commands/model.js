...
export const description = "/model [model_name] - Set or show the target model for chat";
...
export async function execute(remainder, registry) {
 ...
 const buildModelTree = () => {
  ...
  const sortedProviders = Object.keys(modelsByProvider).sort((a, b) => a.localeCompare(b));
  ...
  for (const provider of sortedProviders) {
   ...
   const coldCount = providerModels.filter(m => m.status === 'cold').length;
   const totalCount = providerModels.length;
   const statusIcon = onlineCount > 0
    ? '✅'
    : coldCount > 0
     ? '🧊'
     : '🔴';
