...
export function tokenRingTool({ name,  description, inputSchema, execute}: TokenRingToolDefinition) {
  return {
    name,
    tool: aiTool({
      description,
      inputSchema,
      async execute(args: Record<string, any>, {experimental_context}: Record<string, any>): Promise<string | object> {
        ...
        return await agent.getState(AIServiceState).runToolMaybeInParallel(executeToolFunction);
      }
    })
  }
}