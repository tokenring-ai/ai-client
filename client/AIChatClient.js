...
export default class AIChatClient {
 ...
 calculateCost({promptTokens, completionTokens}) {
  if (!this.modelSpec || promptTokens === undefined || completionTokens === undefined) {
   return undefined;
  }
 ...
 async streamChat(request, registry) {
  if (request.model) throw new Error("streamChat does not accept a model parameter");
  ...
  if (!isHot) {
   chatService.systemLine("Model is not hot and will need to be cold started. Setting retries to 15...");
  }
 ...
 async textChat(request, registry) {
  if (request.model) throw new Error("textChat does not accept a model parameter");
