...
export async function execute({ input, systemPrompt, model }, registry) {
  ...
  if (!model) {
   throw new Error('[runChat] No model parameter received, using the chatService model');
  }
  ...
  try {
    ...
    const chatMessage = chatMessageStorage.storeChat(currentMessage, request, response);
   ...
   if (request) {
    chatMessageStorage.storeChat(currentMessage, request, {error: err.message});
   }
   chatMessageStorage.setCurrentMessage(currentMessage);
   chatService.warningLine("OpenAI response cancelled, restoring prior chat state.");
   throw err;
  }
}
...
export const parameters = {
  ...
  properties: {
    input: {
      type: "array",
      description: "The message content to send to the chat. Can be a string or a properly formatted message array."
    },
    systemPrompt: {
      type: "string",
