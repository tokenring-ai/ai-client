...
export async function init(modelRegistry, config) {
 ...
 if (! generateModelSpec) {
  throw new Error("No config.generateModelSpec provided for Ollama provider.");
 }
 ...
 const getModelList = cachedDataRetriever(baseURL + '/tags', { cacheTime: 60000, timeout: 1000 });
 ...
 for (let modelInfo of modelList.models) {
  ...
  if (type === 'chat') {
   chatModelSpecs[modelInfo.model] = {
    ...
    isHot: () => capabilities.alwaysHot || (
     getRunningModels().then(result => result?.models?.find?.(
