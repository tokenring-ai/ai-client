...
function compactMessageContext(messages) {
 ...
 const MAX_CONTEXT_MESSAGES = parseInt(process.env.MAX_CONTEXT_MESSAGES, 10) || defaultMaxContextMessages;
 ...
 if (messages.length > MAX_CONTEXT_MESSAGES) {
  ...
  if (messagesToKeepCount <= 0) {
   ...
   const recentNonSystemMessages = nonSystemMessages.slice(-messagesToKeepCount);
