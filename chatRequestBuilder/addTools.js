...
export async function addTools(request, registry) {
 for (const { name, description, execute, parameters } of registry.tools.iterateActiveTools()) {
  request.tools[name] = aiTool({
   ...
   execute: async (args, meta) => {
    ...
    try {
     ...
     chatService.errorLine((`Error calling tool ${name}(${JSON.stringify(args)}): ${err}`));
