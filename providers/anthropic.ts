...
export async function init(modelRegistry: ModelRegistry, config: AnthropicModelProviderConfig) {
  ...
  function generateModelSpec(modelId: string, anthropicModelId: string, modelSpec: Omit<ChatModelSpec, "isAvailable" | "providerDisplayName" | "impl" | "modelId">): ChatModelSpec {
    return {
      modelId,
